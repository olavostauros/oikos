...
show_invocation_header() {
  ...
  printf '%s\n' "$task_name"
  if [ "${OIKOS_GIT_SHIM:-}" = "1" ]; then
    info "called as: ${OIKOS_GIT_SHIM_COMMAND:-git preflight}"
    info "git found a PATH executable named ${OIKOS_GIT_SHIM_EXECUTABLE:-git-preflight}; this is a oikos preflight task, not a Git hook"
    if [ -n "${OIKOS_CALLER_PWD:-}" ]; then
      info "default target: caller cwd ($OIKOS_CALLER_PWD)"
    fi
  fi
}
...
resolve_target_path() {
  local path="${1:-}"
  local caller_pwd="${OIKOS_CALLER_PWD:-}"
...
show_repo_identity() {
  ...
  if [ -n "${OIKOS_CALLER_PWD:-}" ]; then
    info "caller cwd: $OIKOS_CALLER_PWD"
  fi
}
