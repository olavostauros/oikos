#!/usr/bin/env bash
# Shared helpers for oikos GitHub issue watch tasks.
#
...
_OIKOS_GITHUB_ISSUES_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1091 # dynamic repo-local source
source "$_OIKOS_GITHUB_ISSUES_LIB_DIR/../common.sh"
...
default_issue_state_dir() {
  ...
  printf '%s/oikos-github-issues-watch/%s/%s\n' \
    "${tmp_root%/}" \
    "$(repo_slug "$repo")" \
    "$(issue_key "$issues")"
}
