...
oikos_welcome_normalize_identity() {
...
oikos_welcome_resident_metadata() {
  ...
  normalized=$(oikos_welcome_normalize_identity "$hint")
...
oikos_welcome_recent_note_changes() {
  local repo_dir="$1"
  local limit="${2:-5}"
...
oikos_welcome_chat_rows() {
  local identity="$1"
  local limit="${2:-5}"
  local payload

  payload=$(chat read oikos --as "$identity" --peek --all --last "$limit" --json 2>/dev/null) || return 1
...
oikos_welcome_github_attention() {
  ...
  work_dir=$(mktemp -d "${TMPDIR:-/tmp}/oikos-welcome-github.XXXXXX") || return 1
