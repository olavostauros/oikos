#!/usr/bin/env python3
"""Smoke tests for the oikos agent mention detector."""
...
def event(body: str, association: str = "MEMBER") -> dict:
    return {
        "repository": {"full_name": "olavostauros/oikos"},
        "issue": {"number": 72, "html_url": "https://github.com/olavostauros/oikos/issues/72"},
        "comment": {
            "html_url": "https://github.com/olavostauros/oikos/issues/72#issuecomment-test",
            "author_association": association,
            "user": {"login": "alpha-oikos"},
            "body": body,
        },
    }
...
def assert_no_suffix_guess(roster: list[str], github_logins: dict[str, str]) -> None:
    for agent in roster:
        ...
        if github_logins[agent] != guessed_login:
            ...
            return
    raise AssertionError("expected at least one configured login not derived from the -oikos suffix")
...
def main() -> int:
    ...
    assert_case("team alias disabled", "@olavostauros/agents hello", [], roster, github_logins)
