...
atrium	https://github.com/olavostauros/atrium.git	0123456789abcdef0123456789abcdef01234567	main
oikos	https://github.com/olavostauros/oikos.git	abcdef0123456789abcdef0123456789abcdef01	main
or-home	https://github.com/olavostauros/home.git	23456789abcdef0123456789abcdef0123456789	main
EOF
}
...
@test "agent:smoke passes for a prepared home with required modules" {
  ...
  add_prepare_modules_env "$home" "atrium oikos"
  add_module_clone "$home" atrium
  add_module_clone "$home" oikos

  run oikos_task agent:smoke --home "$home"

  [ "$status" -eq 0 ]
  [[ "$output" == *$'home notes\tOK\tunlocked/deobfuscated'* ]]
  [[ "$output" == *$'required modules\tOK\tatrium oikos'* ]]
  [[ "$output" == *$'module:oikos notes\tOK\tunlocked/deobfuscated'* ]]
}
...
@test "agent:smoke warns without failing for an unhealthy optional module" {
  ...
  add_notes_ready "$home"
  add_modules_manifest "$home"
  add_prepare_modules_env "$home" "oikos"
  add_module_clone "$home" oikos
  ...
  [[ "$output" == *$'module:oikos clone\tOK\tpresent; required'* ]]
}
...
@test "agent:smoke fails when a required module is locked" {
  ...
  add_prepare_modules_env "$home" "oikos"
  add_module_clone "$home" oikos
  touch "$home/modules/oikos/.locked-notes"

  run oikos_task agent:smoke --home "$home"

  [ "$status" -ne 0 ]
  [[ "$output" == *$'module:oikos notes\tFAIL\tlocked/unknown'* ]]
}
...
@test "agent:smoke fails when required modules are missing" {
  ...
  [ "$status" -ne 0 ]
  [[ "$output" == *$'required modules\tFAIL\tmissing: oikos'* ]]
}
