...
setup() {
  ...
  OIKOS_WELCOME_IDENTITY_DIR="$BATS_TEST_TMPDIR/identities"
  export OIKOS_WELCOME_IDENTITY_DIR

  mkdir -p "$OIKOS_WELCOME_IDENTITY_DIR"
  cat > "$OIKOS_WELCOME_IDENTITY_DIR/gamma.md" <<'MD'
---
title: gamma
type: agent
...
@test "local welcome invokes no Notes chat or GitHub capability" {
  ...
  [ "$status" -eq 0 ]
  [ ! -s "$CAPABILITY_LOG" ]
  [[ "$output" == *"Recent Oikos note changes"* ]]
  [[ "$output" == *"Authoritative notes state"* ]]
  [[ "$output" == *"notes status"* ]]
}

@test "resident welcome uses explicit bounded Oikos chat and verified KKL GitHub attention" {
  CAPABILITY_LOG="$BATS_TEST_TMPDIR/capabilities.log"
  export CAPABILITY_LOG
  : > "$CAPABILITY_LOG"
...
JSON
  ...
  gh() {
    ...
    if [ "${1:-}" = api ] && [ "${2:-}" = graphql ]; then
      ...
      printf '%s\n' '[{"number":164,"title":"Shared orientation","url":"https://example.test/164","repository":{"nameWithOwner":"olavostauros/oikos"},"updatedAt":"now"}]'
...
JSON
  ...
  grep -q '^chat read oikos --as gamma --peek --all --last 5 --json$' "$CAPABILITY_LOG"
  ...
  [ "$(grep -c -- '--owner KnickKnackLabs --owner olavostauros' "$CAPABILITY_LOG")" -eq 3 ]
  [[ "$output" == *"Resident: gamma"* ]]
  [[ "$output" == *"Verified account: gamma-oikos"* ]]
...
@test "visitor welcome never impersonates a resident" {
  ...
  export CAPABILITY_LOG
  : > "$CAPABILITY_LOG"
  export AGENT_NAME=olavostauros
  export CHAT_IDENTITY=olavostauros
  export GIT_AUTHOR_NAME=olavostauros
  ...
  [[ "$output" == *"Visitor: olavostauros"* ]]
  [[ "$output" == *"Verified account: olavostauros"* ]]
}
...
@test "live capability failure preserves the local dashboard" {
  ...
  chat() { return 42; }
  gh() {
    if [ "${1:-}" = api ]; then echo olavostauros; else return 43; fi
  }
  export -f chat gh
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"Recent Oikos note changes"* ]]
  [[ "$output" == *"Oikos repository"* ]]
  [[ "$output" == *"Oikos chat unavailable"* ]]
  [[ "$output" == *"does not match resident gamma-oikos"* ]]
}
...
@test "recent note changes map encrypted ids and remain bounded" {
  ...
  run oikos_welcome_recent_note_changes "$fixture" 5
  [ "$status" -eq 0 ]
