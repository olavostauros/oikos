...
oikos	https://github.com/olavostauros/oikos.git	0123456789abcdef0123456789abcdef01234567	main
atrium	https://github.com/olavostauros/atrium.git	abcdef0123456789abcdef0123456789abcdef01	main
EOF
...
@test "homes:campaign:status reports generic clone details by default" {
  run oikos_task homes:campaign:status --work-dir "$STATE_DIR"
...
@test "homes:campaign:status appends inline check columns" {
  run oikos_task homes:campaign:status \
    --work-dir "$STATE_DIR" \
...
@test "homes:campaign:status appends check-file columns" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *$'Prepare\tNotes\tNotesHooks\tModulesConfig\tModulesManifest\tModule:oikos'* ]]
  [[ "$output" == *$'owner/one\t'*$'\tmain\tclean\tyes\tyes\tpre:yes checkout:no\tyes\treadable\tyes'* ]]
  [[ "$output" == *$'owner/two\t'*$'\tmain\t2 changes\tmissing\tno\tn/a\tno\tunreadable\tunknown'* ]]
  [[ "$output" == *$'owner/encrypted\t'*$'\tmain\tclean\tmissing\tno\tn/a\tno\tencrypted\tencrypted'* ]]
}
...
@test "homes:campaign:status filters by repo" {
  run oikos_task homes:campaign:status --work-dir "$STATE_DIR" --repo owner/two
...
@test "homes:campaign:status fails when no target matches" {
  run oikos_task homes:campaign:status --work-dir "$STATE_DIR" --repo owner/nope
...
@test "homes:campaign:status fails before init" {
  run oikos_task homes:campaign:status --work-dir "$TEST_TMPDIR/absent"
