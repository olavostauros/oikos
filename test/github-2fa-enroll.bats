...
@test "github:2fa:enroll stores enrollment material without printing secrets" {
  ...
  run oikos_task github:2fa:enroll --yes beta

  [ "$status" -eq 0 ]
  [[ "$output" == *"✓ stored TOTP seed and 2 recovery code(s)"* ]]
  [ "$(cat "$FAKE_SECRET_STORE/beta_github-totp")" = "JBSWY3DPEHPK3PXP" ]
  grep -q 'a1b2c-3d4e5' "$FAKE_SECRET_STORE/beta_github-recovery-codes"
  grep -q 'github:2fa:enroll beta --token-name beta --out' "$BATS_TEST_TMPDIR/websites-args"
...
@test "github:2fa:enroll dry-run checks prerequisites without browser automation" {
  ...
  run oikos_task github:2fa:enroll --dry-run theta
...
EOF
  ...
  [ ! -e "$FAKE_SECRET_STORE/beta_github-totp" ]
  [ ! -e "$FAKE_SECRET_STORE/beta_github-recovery-codes" ]
}

@test "github:2fa:enroll requires visible approval" {
  write_fake_websites

  run oikos_task github:2fa:enroll beta
...
EOF
  ...
  [[ "$output" != *"GITHUB_PASSWORD=password-for-beta"* ]]
