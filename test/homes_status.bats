...
oikos_task_stdout_only() {
  oikos_task "$@" 2>"$BATS_TEST_TMPDIR/stderr"
}
...
case "${2:-}" in
  ...
  test-agent/github-username) echo "test-agent-oikos" ;;
...
if [ "${1:-}" = "api" ] && { [ "${2:-}" = "user" ] || [ "${2:-}" = "/user" ]; }; then
  ...
  if [ "${GH_TOKEN:-}" = "fixture-github-token" ]; then
    if [ "${2:-}" = "/user" ]; then
      ...
      printf '{"login":"test-agent-oikos"}\n'
    else
      echo "test-agent-oikos"
    fi
    exit 0
  fi
...
create_ready_home() {
  local home="$1" oikos_pin atrium_pin
...
SH
  ...
  oikos_pin=$(create_module_clone "$home/modules/oikos")
  atrium_pin=$(create_module_clone "$home/modules/atrium")
  printf 'oikos\thttps://github.com/olavostauros/oikos.git\t%s\tmain\n' "$oikos_pin" > "$home/.modules/manifest"
  printf 'atrium\thttps://github.com/olavostauros/atrium.git\t%s\tmain\n' "$atrium_pin" >> "$home/.modules/manifest"
...
[env]
AGENT_PREPARE_MODULES = "atrium oikos"
TOML
  printf 'or-home\thttps://github.com/olavostauros/home.git\t1111111111111111111111111111111111111111\tmain\n' >> "$home/.modules/manifest"
...
@test "homes:status reports a ready configured home" {
  ...
  run oikos_task homes:status test-agent --agents-root "$AGENTS_ROOT" --home "$home"
...
@test "homes:status accepts tracked modules and skips modules outside AGENT_PREPARE_MODULES" {
  ...
  create_ready_home "$home"
  configure_ready_auth
  advance_module_to_tracked_main "$home/modules/oikos" "$BATS_TEST_TMPDIR/oikos.git"
  add_prepare_modules_and_optional_or_home "$home"

  run oikos_task_stdout_only homes:status test-agent --agents-root "$AGENTS_ROOT" --home "$home" --json --check

  [ "$status" -eq 0 ]
  assert_json_output
  [[ "$output" == *'"ready":true'* ]]
  [[ "$output" == *'"name":"Prepare modules","status":"ok","detail":"AGENT_PREPARE_MODULES=atrium oikos"'* ]]
  [[ "$output" == *'"name":"module:oikos","status":"ok","detail":"tracking main at '* ]]
...
@test "homes:status fails when a prepared module has an untrusted mise config" {
  ...
  create_ready_home "$home"
  configure_ready_auth
  printf '[tools]\nnode = "20"\n' > "$home/modules/oikos/mise.toml"
  touch "$home/modules/oikos/.mise-untrusted"
  ...
  assert_json_output
  [[ "$output" == *'"ready":false'* ]]
  [[ "$output" == *'"name":"module-mise:oikos","status":"fail","detail":"untrusted mise config"'* ]]
  [[ "$output" == *"cd $home/modules/oikos && mise trust"* ]]
}
...
@test "homes:status --json --check emits clean ready JSON" {
  ...
  run oikos_task homes:status test-agent --agents-root "$AGENTS_ROOT" --home "$home" --json --check
...
@test "homes:status points missing homes at adopt-remote when auth is ready" {
  ...
  run oikos_task_stdout_only homes:status test-agent \
