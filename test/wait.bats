...
@test "wait rejects a request without a source" {
  run oikos_task wait --state-dir "$STATE_DIR"
...
@test "wait requires an explicit identity for Chat" {
  run oikos_task wait --chat ops --state-dir "$STATE_DIR"
...
@test "wait rejects Chat options without a room" {
  run oikos_task wait --from or --state-dir "$STATE_DIR"
...
@test "wait requires an independent state directory" {
  run oikos_task wait --session alpha
...
@test "wait rejects a state path that is not a directory" {
  ...
  run oikos_task wait --session alpha --state-dir "$STATE_DIR"
...
printf '%s\n' \
  ...
  '{"sender":"alpha","timestamp":"2026-08-22 17:03","body":"unrelated"}'
SH
...
SH
  ...
  run --separate-stderr oikos_task wait \
    --session alpha \
    --state-dir "$STATE_DIR" \
    --timeout 5
...
@test "wait reports a missing source executable" {
  ...
  run -127 --separate-stderr oikos_task wait \
    --session alpha \
    --state-dir "$STATE_DIR" \
    --timeout 5
