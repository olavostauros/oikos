...
if [ "${1:-}" = "get" ] && [ "${2:-}" = "beta/github-pat" ]; then
  printf 'test-token\n'
  exit 0
fi
...
@test "homes:campaign:init clones repos and records targets" {
  run oikos_task homes:campaign:init \
    --work-dir "$TEST_TMPDIR/state" \
    --branch beta/agent-prepare-module-hooks \
    --repo beta-oikos/home:beta \
    --repo KnickKnackLabs/emails:-

  [ "$status" -eq 0 ]
  [ -d "$TEST_TMPDIR/state/clones/beta-oikos__home/.git" ]
  [ -d "$TEST_TMPDIR/state/clones/KnickKnackLabs__emails/.git" ]
  [ "$(cat "$TEST_TMPDIR/state/clones/beta-oikos__home/.git/branch")" = "beta/agent-prepare-module-hooks" ]

  grep -F $'repo\tauth\tclone\tbranch\tbase' "$TEST_TMPDIR/state/targets.tsv"
  grep -F $'beta-oikos/home\tbeta\t' "$TEST_TMPDIR/state/targets.tsv"
...
@test "homes:campaign:init uses named auth only when requested" {
  ...
  [ "$status" -eq 0 ]
  grep -F 'args: get beta/github-pat' "$SECRET_LOG"
