...
@test "homes:smoke:prepare records real agent:prepare failures" {
  run oikos_task homes:smoke:prepare --work-dir "$STATE_DIR"
...
@test "homes:smoke:state reports module clone presence" {
  mkdir -p "$PASS_CLONE/modules/oikos/.git" "$FAIL_CLONE/modules/atrium/.git"

  run oikos_task homes:smoke:state --work-dir "$STATE_DIR"

  [ "$status" -eq 0 ]
  [[ "$output" == *$'agent\thome_notes\thome_modules\tatrium_clone\tatrium_notes\toikos_clone\toikos_notes'* ]]
...
TSV
  ...
  run oikos_task homes:smoke:summary --work-dir "$STATE_DIR"
