#!/usr/bin/env bats

bats_require_minimum_version 1.5.0
load test_helper

setup() {
  export GIT_AUTHOR_NAME="iota"
  export GIT_AUTHOR_EMAIL="iota@oikos.local"
  export MOCK_GIT_REMOTE_ROOT="$BATS_TEST_TMPDIR/remotes"
  export MOCK_GIT_LOG="$BATS_TEST_TMPDIR/git.log"
  mkdir -p "$MOCK_GIT_REMOTE_ROOT"
  : > "$MOCK_GIT_LOG"
  write_rollout_mock_git
  write_rollout_mock_secrets
}

write_rollout_mock_git() {
  local path="$BATS_TEST_TMPDIR/git"
  local real_git
  real_git=$(command -v git)

  cat > "$path" <<'SH'
#!/usr/bin/env bash
set -euo pipefail
printf 'GH_TOKEN=%s ARGS=' "${GH_TOKEN:-ambient}" >> "${MOCK_GIT_LOG:?}"
printf '%q ' "$@" >> "$MOCK_GIT_LOG"
printf '\n' >> "$MOCK_GIT_LOG"

rewritten=()
for arg in "$@"; do
  case "$arg" in
    https://github.com/*.git)
      repo="${arg#https://github.com/}"
      repo="${repo%.git}"
      rewritten+=("$MOCK_GIT_REMOTE_ROOT/$repo.git")
      ;;
    *)
      rewritten+=("$arg")
      ;;
  esac
done

exec "$REAL_GIT" "${rewritten[@]}"
SH
  chmod +x "$path"
  export REAL_GIT="$real_git"
  export GIT="$path"
}

write_rollout_mock_secrets() {
  local path="$BATS_TEST_TMPDIR/secrets"
  cat > "$path" <<'SH'
#!/usr/bin/env bash
set -euo pipefail
if [ "${1:-}" != "get" ]; then
  echo "unexpected secrets command: $*" >&2
  exit 2
fi
case "${2:-}" in
  epsilon/github-pat) echo "token-epsilon" ;;
  alpha/github-username) echo "alpha-oikos" ;;
  alpha/github-pat) echo "token-alpha" ;;
  eta/github-pat) echo "token-eta" ;;
  *) echo "missing secret: ${2:-}" >&2; exit 1 ;;
esac
SH
  chmod +x "$path"
  export SECRETS="$path"
}

create_remote_repo() {
  local repo="$1" content="$2"
  local work="$BATS_TEST_TMPDIR/work-${repo//\//-}"
  local bare="$MOCK_GIT_REMOTE_ROOT/$repo.git"

  mkdir -p "$(dirname "$bare")"
  git init -q -b main "$work"
  printf '%s\n' "$content" > "$work/mise.toml"
  git -C "$work" add mise.toml
  git -C "$work" \
    -c user.name="fixture" \
    -c user.email="fixture@example.test" \
    -c commit.gpgsign=false \
    commit -q -m "initial"
  git clone -q --bare "$work" "$bare"
}

show_remote_file() {
  local repo="$1" ref="$2" file="$3"
  git --git-dir="$MOCK_GIT_REMOTE_ROOT/$repo.git" show "$ref:$file"
}

@test "shiv:rollout dry-run updates existing pins without pushing" {
  create_remote_repo "epsilon/home" '[settings]
quiet = true

[tools]
"shiv:notes" = "0.8"
"shiv:emails" = "0.5"
'
  run oikos_task shiv:rollout emails 0.6 \
    --repo epsilon/home:epsilon \
    --branch rollout/emails-0.6 \
    --work-dir "$BATS_TEST_TMPDIR/clones" \
    --no-gpg-sign

  [ "$status" -eq 0 ]
  [[ "$output" == *"dependency: updated"* ]]
  [[ "$output" != *"regexp escape sequence"* ]]
  [[ "$output" == *"dry-run: would commit and push rollout/emails-0.6"* ]]
  grep -q 'GH_TOKEN=token-epsilon ARGS=.* clone .*epsilon/home.git' "$MOCK_GIT_LOG"
  run git --git-dir="$MOCK_GIT_REMOTE_ROOT/epsilon/home.git" show-ref --verify refs/heads/rollout/emails-0.6
  [ "$status" -ne 0 ]
}

@test "shiv:rollout --home adds missing dependency and pushes with target token" {
  create_remote_repo "alpha-oikos/home" '[settings]
quiet = true

[tools]
"shiv:notes" = "0.8"
jq = "1.8.1"
'

  run oikos_task shiv:rollout emails 0.6 \
    --home alpha \
    --branch rollout/emails-0.6 \
    --message "deps: add emails 0.6" \
    --work-dir "$BATS_TEST_TMPDIR/clones" \
    --add-missing \
    --no-gpg-sign \
    --yes

  [ "$status" -eq 0 ]
  [[ "$output" == *"dependency: added"* ]]
  [[ "$output" == *"pushed: rollout/emails-0.6"* ]]
  grep -q 'GH_TOKEN=token-alpha ARGS=.* clone .*alpha-oikos/home.git' "$MOCK_GIT_LOG"
  grep -q 'GH_TOKEN=token-alpha ARGS=.* push origin HEAD:refs/heads/rollout/emails-0.6' "$MOCK_GIT_LOG"
  [[ "$(show_remote_file "alpha-oikos/home" "refs/heads/rollout/emails-0.6" mise.toml)" == *'"shiv:emails" = "0.6"'* ]]
  author=$(git --git-dir="$MOCK_GIT_REMOTE_ROOT/alpha-oikos/home.git" log -1 --format='%an <%ae>' refs/heads/rollout/emails-0.6)
  [ "$author" = "iota <iota@oikos.local>" ]
}

@test "shiv:rollout updates pins with tab-separated TOML whitespace" {
  create_remote_repo "epsilon/home" '[tools]
"shiv:emails"	=	"0.5"
'

  run oikos_task shiv:rollout emails 0.6 \
    --repo epsilon/home:epsilon \
    --branch rollout/emails-0.6 \
    --work-dir "$BATS_TEST_TMPDIR/clones" \
    --no-gpg-sign

  [ "$status" -eq 0 ]
  [[ "$output" == *"dependency: updated"* ]]
  [[ "$output" == *'-"shiv:emails"	=	"0.5"'* ]]
  [[ "$output" == *'+"shiv:emails"	= "0.6"'* ]]
}

@test "shiv:rollout processes final plan row without trailing newline" {
  create_remote_repo "epsilon/home" '[tools]
"shiv:emails" = "0.5"
'
  printf 'epsilon/home\tepsilon' > "$BATS_TEST_TMPDIR/plan.tsv"

  run oikos_task shiv:rollout emails 0.6 \
    --plan "$BATS_TEST_TMPDIR/plan.tsv" \
    --branch rollout/emails-0.6 \
    --work-dir "$BATS_TEST_TMPDIR/clones" \
    --no-gpg-sign

  [ "$status" -eq 0 ]
  [[ "$output" == *"repo: epsilon/home"* ]]
  [[ "$output" == *"dependency: updated"* ]]
}

@test "shiv:rollout accepts plan rows from stdin" {
  create_remote_repo "eta/home" '[settings]
quiet = true

[tools]
"shiv:notes" = "0.8"
'
  run bash -c 'printf "eta/home\teta\n" | oikos_task shiv:rollout emails 0.6 --plan - --branch rollout/emails-0.6 --work-dir "$BATS_TEST_TMPDIR/clones" --no-gpg-sign'

  [ "$status" -eq 0 ]
  [[ "$output" == *"dependency: missing"* ]]
  run git --git-dir="$MOCK_GIT_REMOTE_ROOT/eta/home.git" show-ref --verify refs/heads/rollout/emails-0.6
  [ "$status" -ne 0 ]
}

@test "shiv:rollout treats package names as literal TOML keys" {
  create_remote_repo "epsilon/home" '[tools]
"shiv:fooXbar" = "0.5"
'

  run oikos_task shiv:rollout foo.bar 1.0 \
    --repo epsilon/home \
    --branch rollout/foo-bar-1.0 \
    --work-dir "$BATS_TEST_TMPDIR/clones" \
    --no-gpg-sign

  [ "$status" -eq 0 ]
  [[ "$output" == *"dependency: missing"* ]]
  [[ "$(cat "$BATS_TEST_TMPDIR/clones/epsilon__home/mise.toml")" == *'"shiv:fooXbar" = "0.5"'* ]]
}
