...
@test "options-only calls use the configured Oikos test directory and run all gates" {
  run oikos_task test --jobs 1 \
...
@test "the bats mode uses the default target without running other gates" {
  run oikos_task test bats --jobs 1 \
...
@test "an explicit test target takes precedence over the configured default" {
  ...
  run oikos_task test bats --jobs 1 "$target"
...
@test "relative test targets resolve from the repository root" {
  run oikos_task test bats --jobs 1 test/agent_list.bats \
    --filter '^agent:list preserves line-oriented CI roster$'
...
@test "whitespace-bearing explicit test targets remain one argument" {
  ...
  run oikos_task test bats --jobs 2 "$target"
...
@test "public Oikos test path runs separate BATS files concurrently" {
...
BATS
  ...
  run oikos_task test bats "$probe_dir"

  [ "$status" -eq 0 ]
}

@test "public Oikos test path keeps tests within one BATS file serial" {
...
BATS
  ...
  run oikos_task test bats "$target"
