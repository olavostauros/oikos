...
case "${2:-}" in
  delta/github-username) echo "delta-oikos" ;;
  delta/github-pat) echo "token-delta" ;;
...
if [ "${1:-}" = "api" ] && [ "${2:-}" = "user" ]; then
  case "${GH_TOKEN:-ambient}" in
    token-delta) echo "delta-oikos" ;;
    token-alpha) echo "alpha-oikos" ;;
    *) echo "or-operator" ;;
  esac
  exit 0
fi
...
if [ "${1:-}" = "api" ] && [ "${2:-}" = "-X" ] && [ "${3:-}" = "PUT" ]; then
  case "${4:-}" in
    /repos/olavostauros/ideas/collaborators/delta-oikos|/repos/olavostauros/ideas/collaborators/alpha-oikos)
      exit 0
      ;;
  esac
fi

if [ "${1:-}" = "api" ] && [[ "${2:-}" == /repos/olavostauros/ideas/collaborators/*/permission ]]; then
  echo "write"
  exit 0
fi
...
if [ "${1:-}" = "api" ] && [ "${2:-}" = "/user/repository_invitations" ]; then
  ...
  case "${GH_TOKEN:-ambient}" in
    token-delta)
      if [ -z "$jq_filter" ]; then
        printf '%s\n' 321 322 323
      elif [[ "$jq_filter" == *'"olavostauros"'* && "$jq_filter" == *'"ideas"'* ]]; then
        echo "321"
      fi
      ;;
    token-alpha) : ;;
    *)
      if [[ "$jq_filter" == *'"olavostauros"'* && "$jq_filter" == *'"ideas"'* ]]; then
        echo "999"
      fi
      ;;
  esac
...
if [ "${1:-}" = "repo" ] && [ "${2:-}" = "view" ] && [ "${3:-}" = "olavostauros/ideas" ]; then
  case "${GH_TOKEN:-ambient}" in
    token-delta) echo "WRITE" ;;
    token-alpha) echo "no-access" ;;
    *) echo "ADMIN" ;;
  esac
  exit 0
fi
...
case "${1:-}" in
  get)
    key="${2:?key required}"
    case "$key" in
      */github-username) printf '%s-oikos\n' "${key%%/*}" ;;
      */github-password) printf 'password-for-%s\n' "${key%%/*}" ;;
      kappa/github-pat) printf 'ghp_operator\n' ;;
      */github-pat)
        path="$(key_path "$key")"
