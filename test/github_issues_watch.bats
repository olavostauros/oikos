...
if [ "${1:-}" = "issue" ] && [ "${2:-}" = "view" ]; then
  ...
  if [ "$wants_json" = "true" ]; then
    if [ "$version" = "2" ]; then
      comments='[{"author":{"login":"or"},"body":"first"},{"author":{"login":"theta"},"body":"second"}]'
    else
      comments='[{"author":{"login":"or"},"body":"first"}]'
    fi
    printf '{"number":%s,"title":"Mock issue %s v%s","state":"OPEN","updatedAt":"2026-06-22T00:00:0%sZ","url":"https://github.com/olavostauros/example-repo/issues/%s","author":{"login":"theta"},"body":"body v%s","comments":%s}\n' \
...
@test "github:issues:status renders selected issues" {
  ...
  run oikos_task github:issues:status --repo olavostauros/example-repo --issues 13,14

  [ "$status" -eq 0 ]
  [[ "$output" == *"repo: olavostauros/example-repo"* ]]
...
@test "github:issues:baseline records current issue state" {
  ...
  run oikos_task github:issues:baseline --repo olavostauros/example-repo --issues 13 --state-dir "$state_dir"
...
@test "github:issues:watch returns on first update" {
  state_dir="$BATS_TEST_TMPDIR/state"
  export MOCK_ISSUE_VERSION=1
  oikos_task github:issues:baseline --repo olavostauros/example-repo --issues 13 --state-dir "$state_dir"

  export MOCK_ISSUE_VERSION=2
  run oikos_task github:issues:watch --repo olavostauros/example-repo --issues 13 --state-dir "$state_dir" --watch-seconds 1 --sleep-seconds 1
...
@test "github:issues:watch rejects zero sleep seconds" {
  ...
  run oikos_task github:issues:watch --repo olavostauros/example-repo --issues 13 --state-dir "$state_dir" --watch-seconds 1 --sleep-seconds 0
...
@test "github:issues:snapshot writes JSON and text artifacts" {
  ...
  run oikos_task github:issues:snapshot --repo olavostauros/example-repo --issues 13,14 --out-dir "$out_dir"
