...
make_release_repo() {
  ...
  git -C "$work" config user.email "test-agent@oikos.local"
...
install_preflight_shims() {
  ...
  oikos_task git:install-shims --bin-dir "$bin" >/dev/null

  printf '%s\n' "$bin"
}

make_agent_home_with_oikos_module() {
  local agent_home="$BATS_TEST_TMPDIR/agent-home"

  mkdir -p "$agent_home/modules"
  ln -s "$REPO_DIR" "$agent_home/modules/oikos"
...
@test "installed git shims require AGENT_HOME instead of legacy home discovery" {
  ...
  mkdir -p "$legacy_home/agents/theta/home/modules"
  ln -s "$REPO_DIR" "$legacy_home/agents/theta/home/modules/oikos"

  run env \
    AGENT_HOME= \
    HOME="$legacy_home" \
    GIT_AUTHOR_NAME=theta \
    GIT_AUTHOR_EMAIL=theta@oikos.local \
    "$bin/git-pre-push"
...
@test "installed git shims resolve oikos from AGENT_HOME modules" {
  bin="$(install_preflight_shims)"
  agent_home="$(make_agent_home_with_oikos_module)"
  repo="$(make_preflight_repo)"

  run env \
    AGENT_HOME="$agent_home" \
    GIT_AUTHOR_NAME=desk-agent \
    GIT_AUTHOR_EMAIL=desk-agent@oikos.local \
    GIT_COMMITTER_NAME=desk-agent \
    GIT_COMMITTER_EMAIL=desk-agent@oikos.local \
    PATH="$bin:$PATH" \
    bash -c 'cd "$1" && git pre-commit' bash "$repo"

  [ "$status" -eq 0 ]
  [[ "$output" == *"oikos git:pre-commit"* ]]
...
@test "pre-publish accepts semver prerelease tag suffixes" {
  ...
  run oikos_task git:pre-publish --tag v1.2.0-kkl.2 "$repo"
...
@test "pre-publish rejects malformed semver prerelease tag suffixes" {
  ...
  run oikos_task git:pre-publish --tag v1.2.0- "$repo"
