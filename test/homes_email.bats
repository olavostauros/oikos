...
#!/usr/bin/env bash
set -euo pipefail
if [ "${1:-}" = --list-secret-keys ] && [ "${2:-}" = test-agent@oikos.local ]; then
  exit 0
fi
...
case "${1:-}" in
  ...
  account:show)
    ...
    cat <<EOF
Account:       test-agent
Address:       test-agent@oikos.local
Display name:  test-agent
Default:       true
IMAP host:     mail.oikos.local
SMTP host:     mail.oikos.local
...
@test "homes:email:setup dry-run retrieves no secret and writes nothing" {
  run oikos_task homes:email:setup test-agent --home "$HOME_REPO"
...
@test "homes:email:setup provisions and validates one ignored home-local account" {
  ...
  run oikos_task homes:email:setup test-agent --home "$HOME_REPO" --yes

  [ "$status" -eq 0 ]
  [[ "$output" == *"Email ready: test-agent <test-agent@oikos.local>"* ]]
...
@test "homes:email:setup is idempotent without retrieving the password again" {
  oikos_task homes:email:setup test-agent --home "$HOME_REPO" --yes >/dev/null
