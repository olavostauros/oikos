...
TOML
  ...
  mkdir -p "$home/modules/oikos" "$home/modules/atrium"
  cat > "$home/modules/oikos/mise.toml" <<'TOML'
[tools]
node = "20"
TOML
  cat > "$home/modules/atrium/mise.toml" <<'TOML'
[tools]
node = "20"
TOML
}
...
@test "homes:publish-fresh dry-run verifies encrypted blobs without pushing" {
  ...
  run oikos_task homes:publish-fresh test-agent \
    --home "$home" \
    --remote-url "$remote" \
    --no-gpg-sign
...
@test "homes:commit-local dry-run does not create a commit" {
  ...
  run oikos_task homes:commit-local test-agent --home "$home" --no-gpg-sign
...
@test "homes:commit-local --yes commits bootstrap surfaces with agent identity" {
  ...
  run oikos_task homes:commit-local test-agent \
    --home "$home" \
  ...
  [ "$author" = "test-agent <test-agent@oikos.local>" ]
...
@test "homes:adopt-remote redacts tokens in remote output" {
  ...
  run oikos_task homes:adopt-remote test-agent \
    --home "$home" \
    --remote-url "https://x-access-token:ghp_secretfixturetoken@github.com/test-agent/home.git" \
    --no-prepare
...
@test "homes:smoke --skip-mise passes a clean simple home" {
  ...
  run oikos_task homes:smoke test-agent --home "$home" --skip-mise
...
@test "homes:smoke trusts mise configs in the home and present modules" {
  ...
  run oikos_task homes:smoke test-agent --home "$home"

  [ "$status" -eq 0 ]
  grep -F "trust $(cd "$home" && pwd -P)" "$MISE_TRUST_LOG" >/dev/null
  grep -F "trust $(cd "$home/modules/oikos" && pwd -P)" "$MISE_TRUST_LOG" >/dev/null
  grep -F "trust $(cd "$home/modules/atrium" && pwd -P)" "$MISE_TRUST_LOG" >/dev/null
}
...
@test "homes:trust-mise trusts the home and present modules" {
  ...
  run oikos_task homes:trust-mise test-agent --home "$home"
