...
setup() {
  ...
  export FAKE_GITHUB_LOGIN="alpha-oikos"
  write_fake_shell
  write_fake_sessions
  write_fake_gh
}
...
if [ "${1:-}" = api ] && [ "${2:-}" = user ] && [ "${3:-}" = --jq ] && [ "${4:-}" = .login ]; then
  [ "${GH_TOKEN:-}" = alpha-target-token ] || {
    echo "wrong GitHub token" >&2
    exit 1
  }
  printf '%s\n' "${FAKE_GITHUB_LOGIN:-alpha-oikos}"
  exit 0
fi
...
case "${1:-}" in
  as)
    ...
    printf 'export GIT_AUTHOR_NAME=alpha\n'
    printf 'export GIT_AUTHOR_EMAIL=alpha@oikos.local\n'
    printf 'export GIT_COMMITTER_NAME=alpha\n'
    printf 'export GIT_COMMITTER_EMAIL=alpha@oikos.local\n'
    printf 'export GH_TOKEN=alpha-target-token\n'
    printf 'export GIT_CONFIG_COUNT=4\n'
    printf 'export GIT_CONFIG_KEY_0=user.name\n'
    printf 'export GIT_CONFIG_VALUE_0=alpha\n'
    printf 'export GIT_CONFIG_KEY_1=user.email\n'
    printf 'export GIT_CONFIG_VALUE_1=alpha@oikos.local\n'
...
make_repo() {
  ...
  if [ "$name" = home ]; then
    git -C "$repo" config user.name alpha
    git -C "$repo" config user.email alpha@oikos.local
  else
  ...
  if [ "$name" = home ]; then
    ...
    git -C "$repo" remote add origin https://github.com/alpha-oikos/home.git
  fi
}
...
@test "agent:desk:prepare dry-run plans an existing desk without mutation" {
  ...
  run oikos_task agent:desk:prepare alpha \
    --desk "$desk" \
    --repo alpha-oikos/home \
    --shell alpha-probe \
  ...
  [[ "$output" == *"repo:    alpha-oikos/home"* ]]
  [[ "$output" == *"dry-run: rerun with --yes"* ]]
  [[ "$output" == *"setup auth: mise run homes:auth:setup alpha --home $desk_path/home --yes"* ]]
  [[ "$output" == *"setup email: mise run homes:email:setup alpha --home $desk_path/home --yes"* ]]
  [[ "$output" == *"mise run agent:desk:wake alpha --desk $desk_path --shell alpha-probe --packet /tmp/packet.md --model '<model>' --yes"* ]]
  [ ! -e "$desk/home" ]
}
...
@test "agent:desk:prepare --yes provisions an existing desk through homes primitives" {
  ...
  auth_line=$(grep -nF "mise run homes:auth:setup alpha --home $desk_path/home --yes" "$MISE_LOG" | cut -d: -f1)
  adopt_line=$(grep -nF "mise run homes:adopt-remote alpha --home $desk_path/home --branch main --yes --repo alpha-oikos/home" "$MISE_LOG" | cut -d: -f1)
  email_line=$(grep -nF "mise run homes:email:setup alpha --home $desk_path/home --yes" "$MISE_LOG" | cut -d: -f1)
  status_line=$(grep -nF "mise run homes:status alpha --home $desk_path/home --json --check" "$MISE_LOG" | cut -d: -f1)
...
@test "agent:desk:prepare --yes can create a desk through desks new" {
  write_fake_mise_for_prepare
  write_fake_desks
  export OIKOS_AGENT_DESK_PREPARE_TIMESTAMP=20260620133700
  ...
  [ "$status" -eq 0 ]
  desk_id="alpha-rewind-investigation-20260620133700"
  desk="$FAKE_DESKS_ROOT/$desk_id"
  grep -F "desks new --id $desk_id" "$DESKS_LOG" >/dev/null
  grep -F "desks path $desk_id" "$DESKS_LOG" >/dev/null
  auth_line=$(grep -nF "mise run homes:auth:setup alpha --home $desk/home --yes" "$MISE_LOG" | cut -d: -f1)
  adopt_line=$(grep -nF "mise run homes:adopt-remote alpha --home $desk/home --branch main --yes --repo alpha-oikos/home" "$MISE_LOG" | cut -d: -f1)
  email_line=$(grep -nF "mise run homes:email:setup alpha --home $desk/home --yes" "$MISE_LOG" | cut -d: -f1)
  [ "$auth_line" -lt "$adopt_line" ]
  [ "$adopt_line" -lt "$email_line" ]
  [[ "$output" == *"desk id: $desk_id"* ]]
  [[ "$output" == *"mise run agent:desk:wake alpha --desk $desk --shell $desk_id --packet /tmp/packet.md --model '<model>' --yes"* ]]
...
@test "agent:desk:status inspects one explicit desk without assuming singleton agent state" {
  desk="$BATS_TEST_TMPDIR/desks/alpha-a"
  mkdir -p "$desk/.desk"
  printf '{"id":"alpha-a"}\n' > "$desk/.desk/registry.json"
  ...
  run oikos_task agent:desk:status alpha --desk "$desk" --shell alpha-a --recent 1
...
JSON
  ...
  run oikos_task agent:desk:pi-auth --pi-dir "$pi_dir"
...
@test "agent:desk:smoke can fail closed when --check is set" {
  ...
  run oikos_task agent:desk:smoke --shell alpha-missing --history-lines 2 --check
...
@test "agent:desk:wake requires an explicit model before rendering" {
  ...
  run oikos_task agent:desk:wake alpha --home "$home" --shell alpha-a --packet "$packet"
...
@test "agent:desk:wake rejects a noncanonical home origin without printing credentials" {
  ...
  packet="$BATS_TEST_TMPDIR/packet.md"
  make_repo "$home" home
  git -C "$home" remote set-url origin https://secret-token@github.com/alpha-oikos/home.git
  printf 'hello packet\n' > "$packet"

  run oikos_task agent:desk:wake alpha \
    --home "$home" \
    --shell alpha-a \
    --packet "$packet" \
...
@test "agent:desk:wake defaults identity to the desk home" {
  ...
  run oikos_task agent:desk:wake alpha --home "$home" --shell alpha-a --packet "$packet" --model openai-codex/gpt-5.6-sol --work-dir "$work_dir"

  [ "$status" -eq 0 ]
  [[ "$output" == *"identity-source: $home_real"* ]]
  [[ "$output" == *"dry-run"* ]]
  [ -x "$work_dir/start-alpha-a.sh" ]
  if [ -f "$SHELL_LOG" ]; then
    ! grep -q 'shell run' "$SHELL_LOG"
  fi
  repo_real=$(cd "$REPO_DIR" && pwd -P)
  grep -q "AGENT_DESK_IDENTITY_SOURCE='$home_real'" "$work_dir/start-alpha-a.sh"
  grep -q "AGENT_DESK_MODEL='openai-codex/gpt-5.6-sol'" "$work_dir/start-alpha-a.sh"
  grep -q "exec '$repo_real/.mise/lib/agent_desk_runtime.sh'" "$work_dir/start-alpha-a.sh"
  ! grep -q 'agent_desk_runtime_scrub_inherited_identity' "$work_dir/start-alpha-a.sh"
  [ "$(wc -l < "$work_dir/start-alpha-a.sh")" -le 20 ]
}
...
@test "agent:desk:wake isolates inherited email selectors to the authenticated desk home" {
  ...
  export EMAILS_CONFIG="/agents/gamma/home/.emails/himalaya.toml"
  export HIMALAYA_CONFIG="/agents/kappa/home/.emails/himalaya.toml"
  export __MISE_DIFF="stale-parent-activation"
  write_target_identity_shimmer

  oikos_task agent:desk:wake alpha --home "$home" --shell alpha-a --packet "$packet" --model openai-codex/gpt-5.6-sol --work-dir "$work_dir" >/dev/null
  run "$work_dir/start-alpha-a.sh"
...
@test "agent:desk:wake scrubs a poisoned parent persona before target activation" {
  ...
  export AGENT_LOG="$agent_log"
  export REQUIRE_CLEAN_PARENT=true
  export AGENT_HOME="/agents/gamma/home"
  export AGENT_IDENTITY=gamma
  export CHAT_IDENTITY=gamma
  export EMAILS_CONFIG="/agents/gamma/home/.emails/himalaya.toml"
  export HIMALAYA_CONFIG="/agents/gamma/home/.emails/himalaya.toml"
  export GH_TOKEN=gamma-token
  export GITHUB_TOKEN=gamma-github-token
  export GH_CONFIG_DIR="/agents/gamma/gh"
  export GIT_AUTHOR_NAME=gamma
  export GIT_AUTHOR_EMAIL=gamma@oikos.local
  export GIT_COMMITTER_NAME=gamma
  export GIT_COMMITTER_EMAIL=gamma@oikos.local
  export GIT_CONFIG_COUNT=1
  export GIT_CONFIG_KEY_0=user.name
  export GIT_CONFIG_VALUE_0=gamma
  export __MISE_DIFF="gamma-activation"
  write_target_identity_shimmer
  ...
  grep -Fx 'AGENT_IDENTITY=alpha' "$agent_log"
  grep -Fx 'CHAT_IDENTITY=alpha' "$agent_log"
  grep -Fx 'GIT_AUTHOR_NAME=alpha' "$agent_log"
  grep -Fx 'GIT_AUTHOR_EMAIL=alpha@oikos.local' "$agent_log"
  grep -Fx 'GIT_COMMITTER_NAME=alpha' "$agent_log"
  grep -Fx 'GIT_COMMITTER_EMAIL=alpha@oikos.local' "$agent_log"
  grep -Fx 'GH_TOKEN=alpha-target-token' "$agent_log"
}
...
@test "agent:desk:wake fails closed on a target GitHub mismatch" {
  ...
  printf 'hello packet\n' > "$packet"
  export AGENT_LOG="$agent_log"
  export FAKE_GITHUB_LOGIN=gamma-oikos
  write_target_identity_shimmer
  ...
  [ "$status" -ne 0 ]
  [[ "$output" == *"GitHub login is gamma-oikos, expected alpha-oikos"* ]]
  [ ! -e "$agent_log" ]
}
...
@test "agent:desk:wake renders relative packet paths as absolute for the launcher" {
  ...
  run oikos_task agent:desk:wake alpha --home "$home" --shell alpha-a --packet AGENTS.md --model openai-codex/gpt-5.6-sol --work-dir "$work_dir"

  [ "$status" -eq 0 ]
  grep -q "AGENT_DESK_PACKET='$repo_real/AGENTS.md'" "$work_dir/start-alpha-a.sh"
}
...
@test "agent:desk:wake --session invokes sessions wake under a scrubbed target identity" {
  ...
  oikos_task agent:desk:wake alpha \
    --session session-123 \
    --home "$home" \
    --shell alpha-cont \
    --packet "$packet" \
  ...
  run "$work_dir/start-alpha-cont.sh"
  ...
  grep -Fx 'AGENT_IDENTITY=alpha' "$SESSIONS_ENV_LOG"
  grep -Fx 'GIT_AUTHOR_NAME=alpha' "$SESSIONS_ENV_LOG"
  grep -Fx 'GIT_AUTHOR_EMAIL=alpha@oikos.local' "$SESSIONS_ENV_LOG"
  grep -Fx 'GIT_COMMITTER_NAME=alpha' "$SESSIONS_ENV_LOG"
  grep -Fx 'GIT_COMMITTER_EMAIL=alpha@oikos.local' "$SESSIONS_ENV_LOG"
  grep -Fx 'GH_TOKEN=alpha-target-token' "$SESSIONS_ENV_LOG"
...
@test "agent:desk:wake --session detects a new process for the same session ID" {
  ...
  [[ "$output" == *"attach:  shell attach alpha-cont"* ]]
  work_real=$(cd "$work_dir" && pwd -P)
  grep -q "shell run --cwd $home_real alpha-cont $work_real/start-alpha-cont.sh" "$SHELL_LOG"
...
@test "agent:desk:wake --session rejects another agent's session before launch" {
  ...
  home_real=$(cd "$home" && pwd -P)
  export FAKE_SESSION_CWD="$home_real"
  export FAKE_SESSION_AGENT=gamma
  export FAKE_TARGET_SESSION_ID=session-123
  ...
  [ "$status" -ne 0 ]
  [[ "$output" == *"session session-123 belongs to gamma, expected alpha"* ]]
  [ ! -e "$SHELL_RUN_MARKER" ]
...
@test "agent:desk:wake --yes waits for a new live Pi process" {
  ...
  run oikos_task agent:desk:wake alpha --home "$home" --shell alpha-a --packet "$packet" --model openai-codex/gpt-5.6-sol --work-dir "$work_dir" --yes

  [ "$status" -eq 0 ]
  [[ "$output" == *"launching fresh wake in shell alpha-a"* ]]
  [[ "$output" == *"Agent wake ready"* ]]
  [[ "$output" == *"mode:    fresh"* ]]
  [[ "$output" == *"session: new-pi-session"* ]]
  [[ "$output" == *"attach:  shell attach alpha-a"* ]]
  work_real=$(cd "$work_dir" && pwd -P)
  grep -q "shell run --cwd $home_real alpha-a $work_real/start-alpha-a.sh" "$SHELL_LOG"
...
@test "agent:desk:wake makes a final observation after a slow deadline-edge probe" {
  ...
  run oikos_task agent:desk:wake alpha --home "$home" --shell alpha-a --packet "$packet" --model openai-codex/gpt-5.6-sol --startup-timeout 1 --work-dir "$work_dir" --yes
...
@test "agent:desk:wake surfaces a shell that exits before Pi starts" {
  ...
  run oikos_task agent:desk:wake alpha --home "$home" --shell alpha-a --packet "$packet" --model openai-codex/gpt-5.6-sol --startup-timeout 5 --work-dir "$work_dir" --yes
