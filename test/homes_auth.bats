...
if [ "${1:-}" = "api" ] && { [ "${2:-}" = "user" ] || [ "${2:-}" = "/user" ]; }; then
  if [ "${GH_TOKEN:-}" = "fixture-github-token" ]; then
    if [ "${2:-}" = "/user" ]; then
      ...
      echo "${FAKE_GH_LOGIN:-test-agent-oikos}"
    fi
    exit 0
  fi
...
@test "homes:auth reports ready when local auth is configured" {
  ...
  run oikos_task homes:auth test-agent --agents-root "$AGENTS_ROOT"
  ...
  [[ "$output" == *"GitHub token"*"valid as test-agent-oikos"* ]]
  [[ "$output" == *"expires in"* ]]
  [[ "$output" == *"# Next"* ]]
  [[ "$output" == *"auth ready"* ]]
...
@test "homes:auth --json reports missing local setup without leaking token" {
  run oikos_task homes:auth test-agent --agents-root "$AGENTS_ROOT" --json
  ...
  [[ "$output" == *"valid as test-agent-oikos; expires in"* ]]
...
@test "homes:auth --json --check exits nonzero with clean JSON" {
  run oikos_task_stdout_only homes:auth test-agent --agents-root "$AGENTS_ROOT" --json --check
...
@test "homes:auth --check exits nonzero when auth is not ready" {
  run oikos_task homes:auth test-agent --agents-root "$AGENTS_ROOT" --check
...
@test "homes:auth:setup dry-run does not import or write local config" {
  run oikos_task homes:auth:setup test-agent --agents-root "$AGENTS_ROOT"
...
@test "homes:auth:setup --yes validates GitHub before mutating local auth" {
  ...
  run oikos_task homes:auth:setup test-agent --agents-root "$AGENTS_ROOT" --yes
...
@test "homes:auth:setup --yes imports key, writes gitconfig, and configures includeIf" {
  ...
  [ "$(git config --file "$AGENTS_ROOT/test-agent/.gitconfig" user.email)" = "test-agent@oikos.local" ]
...
@test "homes:auth:setup --home targets the supplied home parent" {
  ...
  run oikos_task homes:auth:setup test-agent \
  ...
  run oikos_task homes:auth test-agent \
    --agents-root "$AGENTS_ROOT" \
    --home "$custom_home" \
    --json
  [ "$status" -eq 0 ]
